...
const handleFormSubmit = () => { document.getElementById('contact-form').reset(); const fb = document.getElementById('msg-success-box'), sb = document.getElementById('submit-btn'), ab = document.getElementById('draft-email-btn'); if (sb) sb.classList.add('is-hidden'); if (ab) ab.classList.add('is-hidden'); fb.classList.add('anim-start'); setTimeout(() => fb.classList.add('anim-running'), 100); setTimeout(() => { fb.classList.remove('anim-running'); fb.classList.add('anim-reverse'); }, 3000); setTimeout(() => { fb.classList.remove('anim-start', 'anim-reverse'); fb.style.display = 'none'; if (sb) sb.classList.remove('is-hidden'); if (ab) ab.classList.remove('is-hidden'); submitted = false; }, 5000); };
...
const closeTools = () => { const gc = document.getElementById('global-controls'), gt = document.querySelector('.tools-toggle'); if (gc && gc.classList.contains('tools-open')) { gc.classList.remove('tools-open'); if (gt) { gt.classList.remove('open'); gt.setAttribute('aria-expanded', 'false'); } closeAllDropdowns(); } };
const toggleTools = (e) => { if (e) e.stopPropagation(); const gc = document.getElementById('global-controls'), gt = document.querySelector('.tools-toggle'), nb = document.querySelector('.nav-list'), tb = document.querySelector('.nav-toggle'); if (nb && nb.classList.contains('active')) { nb.classList.remove('active'); if (tb) tb.classList.remove('open'); } const opening = !gc.classList.contains('tools-open'); gc.classList.toggle('tools-open', opening); gt.classList.toggle('open', opening); gt.setAttribute('aria-expanded', opening ? 'true' : 'false'); if (!opening) closeAllDropdowns(); };
const toggleMenu = (e) => { if (e) e.stopPropagation(); const nb = document.querySelector('.nav-list'), tb = document.querySelector('.nav-toggle'), mDD = document.getElementById('global-lang-dd'); if (mDD && mDD.classList.contains('show')) mDD.classList.remove('show'); closeTools(); closeAllDropdowns(); nb.classList.toggle('active'); tb.classList.toggle('open'); }; const toggleMobileSubmenu = (e, tId) => { e.stopPropagation(); const dd = document.getElementById(tId), tb = e.currentTarget; closeAllDropdowns(dd); if (dd.classList.contains('show-mobile')) { dd.classList.remove('show-mobile'); tb.classList.remove('active'); tb.innerHTML = '<i class="fas fa-chevron-down"></i>'; } else { dd.classList.add('show-mobile'); tb.classList.add('active'); tb.innerHTML = '<i class="fas fa-chevron-up"></i>'; } }; document.querySelectorAll('.nav-link').forEach(l => { if (!l.closest('.nav-item-dropdown')) l.addEventListener('click', () => { const nb = document.querySelector('.nav-list'), tb = document.querySelector('.nav-toggle'); if (nb.classList.contains('active')) { nb.classList.remove('active'); tb.classList.remove('open'); } }); });
...
const generateSyllabus = (courseKey) => {
    ...
    triggerAIFeature(
        ...
        (res) => {
            showAIModal(`<div class="syllabus-modal-content"><div class="syllabus-header"><span class="syllabus-icon">${course.icon}</span><div style="flex:1;min-width:0;"><div class="syllabus-title">${course.title}</div><div class="syllabus-subtitle">${course.level} &nbsp;·&nbsp; AI-Generated Syllabus Outline</div></div><button onclick="copyAIContent(this,'syllabus-body')" title="Copy to clipboard" style="background:none;border:1px solid #ccc;border-radius:6px;cursor:pointer;color:#666;padding:5px 7px;line-height:0;display:flex;align-items:center;justify-content:center;flex-shrink:0;"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="2" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button></div><div class="syllabus-body" id="syllabus-body">${res}</div><p class="syllabus-disclaimer">AI-generated outline — customise as needed before institutional submission.</p></div>`);
        }
    );
};
